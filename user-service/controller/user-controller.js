...
export async function createUser(req, res) {
    try {
        ...
        if (username && email && password) {
            const existingUser = await _findUserByUsernameOrEmail(
                username,
                email
            );
            if (existingUser) {
            ...
            return res
                ...
                .json({
                    message:
                        "username and/or email and/or password are missing",
                });
        }
    } catch (err) {
        console.error(err);
...
export const sendPasswordResetEmail = async (req, res) => {
    ...
    try {
        ...
        const resetLink = `http://34.124.183.106:3000/auth/reset-password/${token}`;
